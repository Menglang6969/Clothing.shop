...
public class StockServiceImpl implements StockService {
    ...
    @Override
    public Page<StockEntity> getAll(int page, int limit, String sort, String query, Long branch_id) throws Exception {
        Pageable pageable = PageableResponse.mapPageable(page, limit, sort);
        BranchEntity branch = branchRepository.findById(branch_id).orElseThrow(() -> new NotFoundException("Branch Not found"));
        if (!query.isEmpty() || !query.isBlank()) {
            return this.stockRepository.findAllStock(query, branch, pageable);
        }
        return this.stockRepository.findAllStock(null, branch, pageable);
    ...
    @Override
    public Page<StockResponse> getCustomPage(List<StockResponse> stock,Pageable pageable, Long totalElements){
        return new PageImpl<>(stock, pageable, totalElements);
    }
