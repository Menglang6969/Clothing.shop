...
public interface OrderRepository extends JpaRepository<OrderEntity,Long> {
    ...
    @Query("SELECT SUM(o.totalPrice) FROM OrderEntity o WHERE o.createdAt  BETWEEN ?1 AND ?2")
    Double sumTotalPriceBetweenDates(Date startDate, Date endDate);

    @Query("SELECT SUM(o.totalPriceKHR) FROM OrderEntity o WHERE o.createdAt BETWEEN ?1 AND ?2")
    Double sumTotalPriceKHRBetweenDates(Date startDate, Date endDate);

    @Query("SELECT SUM(o.totalBasePrice) FROM OrderEntity o WHERE o.createdAt BETWEEN ?1 AND ?2")
